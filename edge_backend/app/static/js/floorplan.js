  ...
  class BlueprintEditor {
    ...
    renderCameraInspector(panel, title, cam) {
      ...
      panel.innerHTML = `
        ...
        <img id="fpCamThumb" class="fp-thumb" alt="Live frame from ${escapeHtml(cam.name)}" src="/api/v1/cameras/${encodeURIComponent(cam.camera_id)}/snapshot?annotate=false&_=${Date.now()}">
        <div class="fp-kv">
      ...
      this._thumbTimer = setInterval(() => {
        ...
        if (this.isVisible()) img.src = `/api/v1/cameras/${encodeURIComponent(cam.camera_id)}/snapshot?annotate=false&_=${Date.now()}`;
